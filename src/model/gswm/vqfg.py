...
class VQFgModule(nn.Module):
    def __init__(self):
        nn.Module.__init__(self)

        ###### VQ
        self.vq_what_beta = 1.0
        ###### VQ
        ...
        self.pres_depth_where_what_prior = VQPresDepthWhereWhatPrior()
        # VQ
        ...
        self.pres_depth_where_what_latent_post_disc = VQPresDepthWhereWhatPostLatentDisc()
        # VQ
    ...
    def track(self, seq, bg, discovery_dropout):
        ...
        for t in range(T):
            ...
            state_post_prop, state_prior_prop, z_prop, kl_prop, proposal = self.propagate(
                x, state_post, state_prior, z, bg[:, t])
            ids_prop = ids
            if first or torch.rand(1) > discovery_dropout:
                ...
                state_post_disc, state_prior_disc, z_disc, ids_disc = self.get_dummy_things(
                    B, seq.device)
    ...
    def generate(self, seq, bg, cond_steps, sample):
        ...
        for t in range(T):
            ...
            if t < cond_steps:
                ...
                x = seq[:, t]
                # Tracking
                state_post_prop, state_prior_prop, z_prop, kl_prop, proposal = self.propagate(x, state_post, state_prior, z, bg[:, t])
                ids_prop = ids
                if t == 0:
                    state_post_disc, state_prior_disc, z_disc, ids_disc, kl_disc = self.discover(x, z_prop, bg[:, t], start_id)
                else:
    ...
    def discover(self, x, z_prop, bg, start_id=0):
        ...
        x_enc = self.img_encoder(x)  # (B, D, G, G)
        # For each discovery cell, we combine propagated objects weighted by distances
        
        prop_map = self.compute_prop_map(z_prop)  # (B, D, G, G)
        
        enc = torch.cat([x_enc, prop_map], dim=1)  # (B, 2*D, G, G)
        ...
        #****
        # z_what_post_param: (B, G*G, D)
        z_what_q_x_st, z_what_q_x = self.what_codebook.straight_through(
            z_what_post_param.reshape(B*ARCH.G**2, ARCH.Z_WHAT_DIM))
        z_what_q_x_st = z_what_q_x_st.reshape(B, ARCH.G**2, ARCH.Z_WHAT_DIM)
        z_what_q_x = z_what_q_x.reshape(B, ARCH.G**2, ARCH.Z_WHAT_DIM)
        z_what = z_what_q_x_st
        # VQ
        ...
        z_depth_prior, z_where_prior, z_what_prior, z_dyna_prior = self.get_discovery_priors(x.device)  # z_what_prior is unnecessary
        # where prior, (B, G*G, 4)
        ...
        kl_what = self.what_codebook.get_loss(
            z_what_q_x, z_what_post_param, self.vq_what_beta, reduction='none')
        # VQ
    ...
    def propagate(self, x, state_post_prev, state_prior_prev, z_prev, bg):
        ...
        Args:
            x: (B, 3, H, W), img
            (h, c), (h, c): each (B, N, D)
            state_post_prev: (B, N, D)
            state_prior_prev: (B, N, D)
            z_prev:
                z_pres: (B, N, 1)
                z_depth: (B, N, 1)
                z_where: (B, N, 4)
...
class VQPresDepthWhereWhatPrior(nn.Module):
    """p(z_att|z_dyna)"""
    ...
    def __init__(self):
        ...
        self.enc = MLP(
            [ARCH.Z_DYNA_DIM, 128, 128,
             ARCH.Z_PRES_DIM + ARCH.Z_DEPTH_DIM*2 + ARCH.Z_WHERE_DIM*2 + ARCH.Z_WHAT_DIM + (
                     ARCH.Z_DEPTH_DIM + ARCH.Z_WHERE_DIM + ARCH.Z_WHAT_DIM
             )
             ], act=nn.CELU())
        # VQ
    ...
    def forward(self, enc):
        ...
        (z_pres_prob, z_depth_loc, z_depth_scale, z_where_loc, z_where_scale,
         # z_what_loc, z_what_scale,  # VQ
         z_what_param,  # VQ
         z_depth_gate, z_where_gate, z_what_gate) = torch.split(params,
            [ARCH.Z_PRES_DIM] + [ARCH.Z_DEPTH_DIM] * 2 + [
                ARCH.Z_WHERE_DIM] * 2 + 
                # [ARCH.Z_WHAT_DIM] * 2  # VQ
                [ARCH.Z_WHAT_DIM]  # VQ
                + [ARCH.Z_DEPTH_DIM,
                    ARCH.Z_WHERE_DIM,
                    ARCH.Z_WHAT_DIM]
        ...
        z_depth_scale = F.softplus(z_depth_scale) + 1e-4
        z_where_scale = F.softplus(z_where_scale) + 1e-4
        # z_what_scale = F.softplus(z_what_scale) + 1e-4  # VQ
        ...
        # return z_pres_prob, z_depth_loc, z_depth_scale, z_where_loc, z_where_scale, z_what_loc, z_what_scale, z_depth_gate, z_where_gate, z_what_gate
        return z_pres_prob, z_depth_loc, z_depth_scale, z_where_loc, z_where_scale, z_what_param, z_depth_gate, z_where_gate, z_what_gate  # VQ
...
#         params = params.permute(0, 2, 3, 1).view(B, ARCH.G ** 2, -1)
#         (z_pres_prob, z_depth_loc, z_depth_scale, z_where_loc, z_where_scale,
#          z_what_loc, z_what_scale, z_dyna_loc, z_dyna_scale) = torch.split(
#             params,
#                [ARCH.Z_PRES_DIM] + [
#                    ARCH.Z_DEPTH_DIM] * 2 + [
#                    ARCH.Z_WHERE_DIM] * 2 + [
...
class VQPresDepthWhereWhatPostLatentDisc(nn.Module):
    """Predict attributes posterior given image encoding and propagation map in discovery"""
    ...
    def __init__(self):
        ...
        self.enc = nn.Sequential(
            ...
            # VQ
            nn.Conv2d(128,
                      ARCH.Z_PRES_DIM + ARCH.Z_DEPTH_DIM*2 + ARCH.Z_WHERE_DIM*2 + ARCH.Z_WHAT_DIM + ARCH.Z_DYNA_DIM*2,
                      1),
            # VQ
        )
    ...
    def forward(self, x):
        ...
        params = params.permute(0, 2, 3, 1).view(B, ARCH.G ** 2, -1)
        (z_pres_prob, z_depth_loc, z_depth_scale, z_where_loc, z_where_scale,
         z_what_param, z_dyna_loc, z_dyna_scale) = torch.split(  # VQ
            params,
               [ARCH.Z_PRES_DIM] + [
                   ARCH.Z_DEPTH_DIM] * 2 + [
                   ARCH.Z_WHERE_DIM] * 2 + [
        ...
        return z_pres_prob, z_depth_loc, z_depth_scale, z_where_loc, z_where_scale, z_what_param,z_dyna_loc, z_dyna_scale  # VQ
